...
def map_to_nearest_image(pos_center, pos_target, cell):
    """
    Map a target position to the nearest periodic image relative to a center atom.

    This helper assumes an orthogonal cell and shifts the target atom by whole-cell
    translations so that the image closest to pos_center is selected.

    Parameters
    ----------
    pos_center : numpy.ndarray
        Cartesian position of the reference atom.
    pos_target : numpy.ndarray
        Cartesian position of the target atom.
    cell : numpy.ndarray
        Cell matrix.
    ...
    -------
    numpy.ndarray
        Mapped Cartesian position of the target atom.
    """
    vec = pos_target - pos_center
    cell_lengths = np.linalg.norm(cell, axis=1)

    for i in range(3):
        if abs(vec[i]) > cell_lengths[i] / 2.0:
            if vec[i] > 0:
                vec[i] -= cell_lengths[i]
            else:
                vec[i] += cell_lengths[i]

    return pos_center + vec


def rotate_carbon_bond(atoms, active_indices, params, verbose=True):
    """
    Perform a trial rotation on a local carbon-carbon bond environment with PBC support.

    The procedure is:
    1. Randomly choose one active carbon atom A.
    2. Randomly choose two distinct carbon neighbors B and D around A.
    3. Map B and D to their nearest periodic images relative to A.
    4. Rotate the A-B pair by 90 degrees around the normal of the A-B-D plane.
    5. Wrap the rotated structure back into the simulation cell.
    ...
    if use_pbc:
        cell = atoms.get_cell()
        pos_B_mapped = map_to_nearest_image(pos_A, pos_B, cell)
        pos_D_mapped = map_to_nearest_image(pos_A, pos_D, cell)

        if verbose:
            print("  Periodic-image mapping:")
            print(f"    Original B: {pos_B}")
            print(f"    Mapped   B: {pos_B_mapped}")
            print(f"    Original D: {pos_D}")
            print(f"    Mapped   D: {pos_D_mapped}")
    else:
        pos_B_mapped = pos_B
        pos_D_mapped = pos_D

    vec_AB = pos_B_mapped - pos_A
    vec_AD = pos_D_mapped - pos_A
    ...
    rotation_center = (pos_A + pos_B_mapped) / 2.0
    ...
    for idx, pos in [(atom_A_idx, pos_A), (atom_B_idx, pos_B_mapped)]:
        pos_centered = pos - rotation_center
        pos_rotated = np.dot(R, pos_centered)
        new_positions[idx] = pos_rotated + rotation_center
    ...
    if verbose:
        pos_A_new = atoms_rotated.get_positions()[atom_A_idx]
        pos_B_new = atoms_rotated.get_positions()[atom_B_idx]
        print(f"  Rotated A position: {pos_A_new}")
        print(f"  Rotated B position: {pos_B_new}")

        if use_pbc:
            _, dist_new = get_distances(
                [pos_A_new],
                [pos_B_new],
                cell=atoms_rotated.get_cell(),
                pbc=atoms_rotated.get_pbc(),
            )
            print(f"  Rotated A-B distance: {dist_new[0, 0]:.4f} A")
        else:
            dist_new = np.linalg.norm(pos_B_new - pos_A_new)
            print(f"  Rotated A-B distance: {dist_new:.4f} A")
...
def monte_carlo_rotation(params):
    ...
    print("\n" + "#" * 70)
    print("#" + " " * 68 + "#")
    print("#" + " " * 15 + "PBC Monte Carlo Rotation Simulation Start" + " " * 14 + "#")
    print("#" + " " * 68 + "#")
    print("#" * 70 + "\n")
    ...
    for step in range(1, rotate_max + 1):
        ...
        print("Applying trial rotation with PBC-aware mapping...")
    ...
    final_step = step
    print("\n" + "=" * 70)
    print("PBC Monte Carlo Simulation Complete")
    print("=" * 70)

    with open("mc_summary.log", "w", encoding="utf-8") as f:
        f.write("PBC Monte Carlo Rotation Simulation Summary\n")
        f.write("=" * 50 + "\n")
    ...
    print("\n" + "#" * 70)
    print("#" + " " * 68 + "#")
    print("#" + " " * 16 + "PBC Monte Carlo Rotation Simulation End" + " " * 15 + "#")
    print("#" + " " * 68 + "#")
    print("#" * 70 + "\n")
...
def main():
    """
    Run the complete PBC Monte Carlo rotation workflow.
    """
